...
from data import DataRepository, find_available_modules, filter_modules_with_date, Module
...
async def fetch_available_modules(repository: DataRepository):
    ...
    print("📲 Obtendo modules...", end='', flush=True)
    ...
    print(f" Encontrado {sum(len(modules) for modules in all_modules.values())} modulos.", flush=True)

    available_modules = {} | {course: find_available_modules(modules) for course, modules in all_modules.items()}
...
def construct_message(available_modules: dict[str, list[Module]]):
    ...
    available_modules_count = sum(len(modules) for modules in available_modules.values())
